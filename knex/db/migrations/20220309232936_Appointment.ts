...
export async function up(knex: Knex): Promise<void> {
    ...
    .createTable('Appointment',(table:Knex.TableBuilder)=>{
        table.uuid('sAppointmentId').unique().defaultTo(knex.raw('uuid_generate_v4()'));
        table.uuid('sDoctorId').references('sDoctorId').inTable('Doctor').notNullable();
        table.uuid('sPatientId').references('sPatientId').inTable('Patient').notNullable();
